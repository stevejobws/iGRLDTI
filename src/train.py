...
def train(epoch, model, optimizer, feature, record, idx_train, idx_val, idx_test, labels):
    model.train()
    optimizer.zero_grad()
    ...
    optimizer.step()
    model.eval()
    output, Emdebding = model(feature)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--k1', type=int, default=200, help='Value of K in stage (1).') #  stage (1)：feature smoothing with NDLS (NDLS-F),
    parser.add_argument('--k2', type=int, default=20, help='Value of K in stage (3).') # stage (3)：label smoothing with NDLS (NDLS-L)
    parser.add_argument('--epsilon1', type=float, default=0.03, help='Value of epsilon in stage (1).')
    parser.add_argument('--epsilon2', type=float, default=0.05, help='Value of epsilon in stage (2).') # stage (2)：model training with smoothed features
